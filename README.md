...
- 改一句台词，画面自动重新对齐
- 同一套场景代码，一行参数就能换成 10 种风格中的任意一种；横屏、竖屏、方屏都支持
...
- 插画镜头：粗描边、平涂、排线纹理的插画主体，包括圆环隧道、旋臂星系、DNA 双螺旋、神经元、黑洞吸积盘、闪电、棱镜光谱、行星、向日葵，一镜一个大主体
...
| `inked` | 深紫夜色、粗描边插画、排线纹理、星尘 | 钻进物体（portal） |
...
见 `references/motion.md`、`references/generative.md`、`references/illustrate.md`、`references/components.md`、`references/audio.md`。`examples/components` 用一个场景演示一类效果，是最快的上手方式。
...
engine/
  ...
  illustrate.js          插画主体（隧道、星系、双螺旋、神经元、黑洞、闪电、棱镜、行星、向日葵）
  styles/*.js            10 个风格包
scripts/
  init.mjs               新建项目 / 更新引擎
...
examples/
  ...
  token-journey/         50 秒插画短片：一个 token 的旅程
